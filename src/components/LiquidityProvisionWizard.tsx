...
const LiquidityProvisionWizard = ({ onComplete }: { onComplete?: () => void }) => {
  ...
  const handleNextStep = () => {
    if (step === 1) {
      ...
      if (!risksAccepted) {
        toast.error("Please acknowledge the risks associated with providing liquidity");
        return;
      }
  ...
  const handleDeposit = (tokenType) => {
    ...
    toast.success(`${tokenType === 'ownToken' ? liquidityDetails.ownToken : liquidityDetails.pairingToken} deposited successfully`);
  };
    ...
    <div className="wizard-container min-h-[25rem] relative">
      ...
      <div className="wizard-steps flex items-center justify-center mb-8">
        {[1, 2, 3, 4, 5, 6].map((stepNumber) => (
          <div
            key={stepNumber}
            className={`relative flex items-center ${stepNumber < 6 ? 'flex-1' : ''}`}
          >
            <div
            ...
            {stepNumber < 6 && (
              <div
                className={`h-0.5 flex-1 mx-1 transition-all duration-300 ${
                  step > stepNumber ? 'bg-gradient-to-r from-accent-aurora-green to-accent-glow-cyan shadow-nm-sm' : 'bg-white/10'
                }`}
              ></div>
            )}
      ...
      {step === 1 && (
        <div className="wizard-step animate-fade-in">
          <h3 className="text-xl font-semibold mb-2">Select Tokens for Liquidity Pool</h3>
          <p className="wizard-subtitle mb-6">
            You're about to create a new liquidity pool on xExchange. This will make your token tradable.
          </p>
          
          <div className="space-y-6">
            <div className="space-y-2">
              <Label htmlFor="ownToken" className="font-medium">Your Token</Label>
              <Select
              ...
              {liquidityDetails.ownToken && (
                <div className="text-xs text-muted-foreground mt-1 flex items-center">
                  <span className="font-medium mr-1">Your balance:</span>
            ...
            <div className="space-y-2">
              <Label htmlFor="pairingToken" className="font-medium">Pair With</Label>
              <Select
                value={liquidityDetails.pairingToken}
                onValueChange={(value) => handleSelectChange('pairingToken', value)}
              >
                <SelectTrigger id="pairingToken" className="w-full">
              ...
              <div className="text-xs text-muted-foreground mt-1 flex items-center">
                <span className="font-medium mr-1">Your balance:</span>
                {liquidityDetails.pairingToken === 'EGLD' ? '10.5' : '5,000'} {liquidityDetails.pairingToken}
              </div>
            </div>
      ...
      {step === 2 && (
        <div className="wizard-step animate-fade-in">
          <h3 className="text-xl font-semibold mb-2">Define Pool Amounts & Starting Price</h3>
          <p className="wizard-subtitle mb-6">
            Specify the initial amounts of each token you wish to contribute to the pool. The ratio of these amounts will determine the starting price of your token on xExchange.
          </p>
          
          <div className="space-y-6">
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <Label htmlFor="ownTokenAmount" className="font-medium">
                  Amount of {liquidityDetails.ownToken} to Pool
                </Label>
                <button 
                  className="text-xs text-primary font-medium hover:text-primary/80"
                  onClick={() => setLiquidityDetails(prev => ({...prev, ownTokenAmount: "1000000"}))}
                >
                  Max
                </button>
              </div>
              <div className="relative">
                ...
                <div className="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
                  <span className="text-sm text-muted-foreground">{liquidityDetails.ownToken}</span>
                </div>
              </div>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <Label htmlFor="pairingTokenAmount" className="font-medium">
                  Amount of {liquidityDetails.pairingToken} to Pool
                </Label>
                <button 
                  className="text-xs text-primary font-medium hover:text-primary/80"
                  onClick={() => setLiquidityDetails(prev => ({...prev, pairingTokenAmount: liquidityDetails.pairingToken === 'EGLD' ? "5" : "2500"}))}
                >
                  Max
                </button>
              </div>
              <div className="relative">
                ...
                />
                <div className="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
                  <span className="text-sm text-muted-foreground">{liquidityDetails.pairingToken}</span>
                </div>
              </div>
              <div className="text-xs text-muted-foreground">
                ~$10,000.00
            ...
            <Card className="mt-6 overflow-hidden">
              <CardContent className="p-0">
                <div className="p-4 bg-gradient-to-r from-accent/5 to-primary/5 border-b border-white/10">
                  <h4 className="font-medium mb-1">Initial Price Preview</h4>
                  <p className="text-sm text-muted-foreground">Based on your provided amounts</p>
                </div>
                <div className="p-5 text-center">
                  ...
                  <p className="text-sm text-muted-foreground">
                    {liquidityDetails.pairingToken === 'EGLD' && `~$${(parseFloat(calculateInitialPrice()) * 2000).toFixed(2)} USD`}
                  </p>
                </div>
              </CardContent>
            </Card>
      ...
      {step === 3 && (
        <div className="wizard-step animate-fade-in">
          <h3 className="text-xl font-semibold mb-2">Liquidity Pools & LP Tokens Explained</h3>
          <p className="wizard-subtitle mb-4">
            Please read this important information about providing liquidity.
          </p>
          
          <Card className="mb-6">
            <CardContent className="p-5 space-y-4">
              <div>
                <h4 className="font-medium mb-2 text-primary">What you are doing:</h4>
                <p className="text-sm">
                  By depositing your tokens, you're creating a new liquidity pool. This allows others to trade your token against {liquidityDetails.pairingToken} on xExchange.
                </p>
              </div>
              
              <div>
                <h4 className="font-medium mb-2 text-primary">What you receive:</h4>
                <p className="text-sm">
                  In return for providing liquidity, you'll receive LP (Liquidity Provider) Tokens. These LP tokens represent your 100% share of this new pool (since you're the first). They can typically earn trading fees generated by the pool and can be redeemed later to get your underlying tokens back (though amounts may vary).
                </p>
              </div>
              
              <div>
                <h4 className="font-medium mb-2 text-warning-600">Key Risks (Impermanent Loss):</h4>
                <p className="text-sm">
                  Providing liquidity involves risks. The value of your deposited tokens can change relative to holding them separately, a concept known as Impermanent Loss. This means you might withdraw a different ratio of tokens than you deposited, potentially resulting in a lower overall USD value compared to just holding the original tokens. This risk is higher with volatile tokens.
                </p>
                <Button 
                  variant="link" 
                  className="p-0 h-auto flex items-center text-xs mt-2 text-primary"
                >
                  <span>Learn more about Impermanent Loss</span>
          ...
          <div className="bg-card/70 shadow-nm-sm rounded-lg p-4 border border-white/5 mb-6">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h4 className="font-medium">Estimated LP Tokens:</h4>
                <p className="text-2xl font-bold">{expectedLpTokens.toLocaleString()} LP</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-start space-x-3 p-4 bg-warning-50/10 border border-warning-500/20 rounded-lg">
            <Checkbox
              ...
              onCheckedChange={(checked) => setRisksAccepted(!!checked)}
            />
            <Label htmlFor="acceptRisks" className="text-sm cursor-pointer">
              I acknowledge that I have read and understood the risks of providing liquidity, including the potential for Impermanent Loss.
            </Label>
          </div>
      ...
      {step === 4 && (
        <div className="wizard-step animate-fade-in">
          <h3 className="text-xl font-semibold mb-2">Prepare Your Tokens for Pooling</h3>
          <p className="wizard-subtitle mb-6">
            To create the pool, you first need to deposit your specified token amounts into the xExchange smart contract. These actions require separate wallet confirmations.
          </p>
            ...
            <div className="bg-card/70 shadow-nm-sm rounded-lg p-4 border border-white/5">
              <div className="flex items-center justify-between">
                <div className="flex items-center">
                  ...
                  <div>
                    <h4 className="font-medium">Deposit {liquidityDetails.ownTokenAmount} {liquidityDetails.ownToken}</h4>
                  </div>
                </div>
                ...
                {depositSteps.ownToken ? (
                  ...
                  <Button onClick={() => handleDeposit('ownToken')} size="sm">
              ...
              <div className="bg-card/70 shadow-nm-sm rounded-lg p-4 border border-white/5">
                <div className="flex items-center justify-between">
                  <div className="flex items-center">
                    ...
                    <div>
                      <h4 className="font-medium">Deposit {liquidityDetails.pairingTokenAmount} {liquidityDetails.pairingToken}</h4>
                    </div>
                  </div>
                  ...
                  {depositSteps.pairingToken ? (
                    ...
                    <Button onClick={() => handleDeposit('pairingToken')} size="sm">
                      Deposit {liquidityDetails.pairingToken}
                    </Button>
                  )}
                </div>
              </div>
            ...
            <CardContent className="p-0">
              <div className="border-b border-white/5">
                <div className="p-4 grid grid-cols-3 items-center">
                  <div className="text-sm text-muted-foreground">Creating Pool</div>
              ...
              <div className="border-b border-white/5">
                <div className="p-4 grid grid-cols-3 items-start">
                  <div className="text-sm text-muted-foreground">You are Depositing</div>
                  <div className="col-span-2 space-y-1">
                    <div className="font-medium">{liquidityDetails.ownTokenAmount} {liquidityDetails.ownToken}</div>
                    <div className="font-medium">{liquidityDetails.pairingTokenAmount} {liquidityDetails.pairingToken}</div>
                  </div>
                </div>
              </div>
              <div className="border-b border-white/5">
              ...
              <div className="border-b border-white/5">
                <div className="p-4 grid grid-cols-3 items-center">
                  <div className="text-sm text-muted-foreground">Estimated LP Tokens</div>
                  <div className="col-span-2 font-medium">
              ...
              <div className="bg-accent/5 border-t border-white/10">
                <div className="p-4 grid grid-cols-3 items-center">
                  ...
                  <div className="col-span-2 font-medium">~0.0015 EGLD</div>
                </div>
              </div>
            </CardContent>
          </Card>
      ...
      {step === 5 && (
        <div className="wizard-step animate-fade-in flex flex-col items-center justify-center min-h-[25rem]">
          ...
          <h3 className="text-xl font-semibold mb-2">Creating Liquidity Pool</h3>
          <p className="text-center text-muted-foreground mb-4">
            Please confirm the transaction in your connected wallet to create the pool and add liquidity.
          </p>
          <p className="text-sm text-center max-w-sm">
            This transaction will finalize the creation of your liquidity pool on xExchange.
            {liquidityDetails.pairingToken === 'EGLD' && ' Your EGLD will be sent with this transaction.'}
          </p>
        </div>
      )}
      ...
      {step === 6 && (
        <div className="wizard-step animate-fade-in">
          <div className="flex flex-col items-center justify-center mb-8">
            ...
            <h3 className="text-2xl font-semibold mb-1">Success!</h3>
            <p className="text-lg text-center">
              Your <span className="font-medium">{liquidityDetails.ownToken} / {liquidityDetails.pairingToken}</span> liquidity pool has been successfully created and funded on xExchange. Your token is now tradable!
            </p>
          </div>
          ...
          <Card className="mb-6 overflow-hidden">
            <div className="border-b border-white/5">
              <div className="p-4 grid grid-cols-3 items-center">
                <div className="text-sm text-muted-foreground">LP Tokens Received</div>
                <div className="col-span-2 font-medium">
                  {(expectedLpTokens - lpFee).toLocaleString()} LP
                </div>
              </div>
            </div>
            <div className="border-b border-white/5">
              <div className="p-4 grid grid-cols-3 items-center">
                <div className="text-sm text-muted-foreground">LP Token Identifier</div>
                <div className="col-span-2 font-medium">
            ...
            <div className="p-4">
              <p className="text-sm text-muted-foreground">
                These LP tokens represent your share of the liquidity pool. You can use them to withdraw your liquidity in the future.
              </p>
            </div>
          </Card>
          
          <div className="flex flex-col space-y-3">
            <Button variant="default" className="justify-center py-6">
              View Pool on xExchange
              <ExternalLink className="ml-2 h-4 w-4" />
            </Button>
            
            <Button variant="outline" onClick={() => onComplete && onComplete()} className="justify-center">
